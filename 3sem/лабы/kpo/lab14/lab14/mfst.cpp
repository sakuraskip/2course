...
namespace MFST
{
	...
	Mfst::Mfst(Analysis::LEX plex, GRB::Greibach pgrebach)
	{
		grebach = pgrebach;
		lex = plex;
		lenta = new short[lenta_size = lex.lxtable.size];		// массив для ленты, состоящией из символов таблицы лексем
		for (int k = 0; k < lenta_size; k++) lenta[k] = TS(lex.lxtable.table[k].lexema);	// заполнение массива терминалами
		lenta_position = 0;
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d, %s", err.id, lex.lxtable.table[lpos].sn, err.message);
			rc = buf;
		};
		return rc;
	};
